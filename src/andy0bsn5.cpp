...
struct town {
	...
	void init_rand_part_hoxa(int nseq, int *xporti, int nind, int olen, int *len_octa, int *len, int **octa_prowb);
...
void town::init_rand_part_hoxa(int nseq, int *xporti, int nind, int olen, int *len_octa, int *len, int **octa_prowb)
{
	int i, j;
...
/*
int MutRegShiftHoxa0(town *a, int n_train, int *len, int *xporti, int olen, int &npeak, int &nori, int &npos, double *thr_octa, double **octa_prow)
{
	//printf("In Peak %d Ori %d Pos %d ",npeak,nori,npos);
	int r1, r2, i, cy = 10;
	for (i = 0; i < cy; i++)
	{
		...
		if (octa_prow[nori][npeak][npos] > thr_octa[npeak])return 1;
	}
	return -1;
}*/
...
void ReadSeq(char *file, int nseq, int *len, int ***seq_real, int olen, double *octaf, int *octa1, int octa, int octa_size, double **octa_pro1)
{
...
void ReadSeqBack(char *file, int nseq, int *len, int ***seq_back, int olen, int reg_max, double **dav, double **dcv, double *octaf, int *octa1, int octa, int octa_size)
{
	char l[SEQLEN], d[SEQLEN], head[400];
	int m, i, j, k, v, fl = 0;
	int n_backr[MOTLEN];
...
//	double **frp;//LPD frequencies
	...
	ReadSeq(pfile_for, nseq, len, seq_real, olen_min, octa_av, octa1, octa, octa_size, octa_pro1);
	for (i = 0; i < octa_size; i++)octa_rat[i] = log10(octa_av[i]);
	for (i = 0; i < octa_size; i++)octa_av[i] = 0;
	for (i = 0; i < octa_size; i++)octa1[i] = 0;
	ReadSeqBack(pfile_back, nseqb, lenb, seq_back, olen_min, reg_max, dav, dcv, octa_av, octa1, octa, octa_size);
	for (i = 0; i < octa_size; i++)octa_rat[i] -= log10(octa_av[i]);
	/*printf("Octa_rat");
	for (i = 0; i < octa_size; i++)
	{
			...
			for (iter = 0; iter < iteration; iter++)
			{
				...
				//			printf("Poo Ini1 %d\n", i);							
							...
							{	
								err = 1;
								int gom = 0;
								det1.init_rand_hoxa(nseq, olen, size0, reg_max, len_octa, len, octa_prowb);
		...
		//							printf("Poo Ini2 %d\n", i);
			...
			//						printf("Poo Ini3 %d\n", i);
				...
				//	printf("Poo Ini2\n");
					...
					int elit_rec;
					int sr_step = 200 * n_train_max;//100000 if n_train_max = 500
					int n_rec_cycle_max = 4 * sr_step;
					double jwei;
					if (restart == 0)
					{
						step = 4 * n_train_max;//2000 if n_train_max = 500
						step_max = 5 * step;
						elit_rec = ELIT / 4;
						jwei = 1.1;
						mege_h = MEGE;
					...
					else
					{
						...
						{
							jwei = 1.2;
							step = 50 * n_train_max;//25000 if n_train_max = 500
							step_max = 10 * step;
							elit_rec = ELIT / 5;
							mege_h = ELIT;
						}
						...
						{
							step = 100 * n_train_max;//50000 if n_train_max = 500
							step_max = 10 * step;
							elit_rec = 0;
							jwei = 1.3;
						}
					...
					if (restart <= 1)
					{
						if (restart == 0)
						{
							...
							for (i = half; i < ELIT; i++)
							{
								...
								pop[iter][i].init_rand_part_hoxa(n_train[iter], xporti, 20, olen, len_octa, len, octa_prowb);
